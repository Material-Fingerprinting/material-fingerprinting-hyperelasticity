...
class Material():
    ...
    def scale_parameters(self,parameters,factor):
        ...
        for i in range(self.n_parameters):
            if self.dim_parameters[i] == "dimensionless":
                pass
            elif self.dim_parameters[i] == "force/area":
                parameters[i] = parameters[i] * factor
            else:
...
def get_error_strain_energy_density_incompressible_lam(mat_true,parameters_true,mat_disc,parameters_disc):
    ...
    def integrand_W_diff(lam1,lam2):
        W_true = mat_true.get_strain_energy_density_incompressible_lam(parameters_true,lam1,lam2)
        W_disc = mat_disc.get_strain_energy_density_incompressible_lam(parameters_disc,lam1,lam2)
    ...
    integral_diff, _ = scipy.integrate.dblquad(integrand_W_diff, x_lower, x_upper, y_lower, y_upper)
