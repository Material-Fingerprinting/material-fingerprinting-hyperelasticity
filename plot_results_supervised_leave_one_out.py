...
for model, param in models.items():
    ...
    if save: fig.savefig(f'plots/fingerprints_loo_{model_abbrev}.png', bbox_inches='tight')
    if close: plt.close(fig)

    ## === Plot stress strain for NOISE 2 ====
    # !!! brute force color change !!!
    if model_abbrev == 'BlatzKo':
        color_stress_strain = colors['MooneyRivlin']
    elif model_abbrev == 'Demiray':
        color_stress_strain = colors['Holzapfel']
    elif model_abbrev == 'MooneyRivlin':
        color_stress_strain = colors['Ogden']
    elif model_abbrev == 'NeoHooke':
        color_stress_strain = colors['Ogden']
    elif model_abbrev == 'Ogden':
        color_stress_strain = colors['Demiray']
    else:
        color_stress_strain = colors[model_abbrev]

    fig1, ax1 = plt.subplots(1,2)
    # plot results for uniaxial tension
    print("        ---> mat_noise_005:" , mat_noise_005.name)
    ...
    fig1.tight_layout()
    if show: plt.show(block=False)
    if save: fig1.savefig(f'plots/stress_strain_loo_noise2_{model_abbrev}.png', bbox_inches='tight', dpi=1000)
